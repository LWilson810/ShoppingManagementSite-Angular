...
import {NewFileViewModel, UploadResponseViewModel, NewFilesViewModel} from '../models/new-file.model';
...
export class HttpHelperService {
  ...
  public authJsonDelete<T>(url: string, data:any, params: HttpParams = null, shouldDisplayProgresse: boolean = true): Observable<T> {
    const headers = this.authService.authJsonHeaders();
    return this.request<T>(HtttpMethodType.delete, url, headers, data, params, shouldDisplayProgresse);
  }
  ...
  public authUploadMultiFilesAsync(url: string, data: NewFilesViewModel, params: HttpParams = null, shouldDisplayProgresse: boolean = true) {

    const formData: FormData = new FormData();
    this.multiObjectToFormData(data, formData, null);
  ...
  protected handleError(response: Response | any) {
    ...
    if (response.status === 403) {
      ...
      this.notificationService.error('You do not have permission', response.error.text);
  ...
  protected multiObjectToFormData(objects: NewFilesViewModel, form?: FormData, namespace?: string) {
    // console.log(objects);
    for (var i = 0; i < objects.files.length; i++ ) {
      form.append('files', objects.files[i], objects.files[i].name);
    }
    // console.log(form);
    return form;
  }
